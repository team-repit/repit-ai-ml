...
# 파일 경로 설정
# ─────────────────────────────────────────────────────────────────────────────
input_path = "lunge_video.mp4"
output_path = "lunge_output_angle_based.mp4"
...
with mp_pose.Pose(static_image_mode=False, model_complexity=1, min_detection_confidence=0.5, smooth_landmarks=True) as pose_video:
    while cap.isOpened():
        ...
        if (front_knee_angle_prev2 is not None and front_knee_angle_prev1 is not None and front_knee_angle_curr is not None and
                back_knee_angle_prev1 is not None and hip_angle_prev1 is not None):
            if front_knee_angle_prev2 > front_knee_angle_prev1 < front_knee_angle_curr:
        ...
        if display_countdown > 0:
            ...
            font_face, font_scale, thickness, color = cv2.FONT_HERSHEY_SIMPLEX, 1.0, 2, (0, 0, 255)
            (text_width, text_height), baseline = cv2.getTextSize(overlay_text, font_face, font_scale, thickness)
            x_pos, y_pos = (frame_width - text_width) // 2, text_height + 20
