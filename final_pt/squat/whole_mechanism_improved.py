...
class SquatPhaseDetector:
    ...
    def detect_phase(self, knee_angle: float, velocity: float, frame_count: int) -> str:
        ...
        if abs(velocity) < self.velocity_threshold:
            ...
            if self.current_phase in ['preparation', 'completion']:
...
def evaluate_advanced_squat_grade(
    ...
    # 등급 할당 - 더 관대한 기준으로 조정
    if phase == 'bottom':
        if total_score >= 0.75:  # 0.9에서 0.75로 감소
            return "A"
        elif total_score >= 0.65:  # 0.8에서 0.65로 감소
            return "B"
        elif total_score >= 0.55:  # 0.7에서 0.55로 감소
            return "C"
        elif total_score >= 0.45:  # 0.6에서 0.45로 감소
            return "D"
        else:
            return "F"
    else:
        if total_score >= 0.70:  # 0.85에서 0.70으로 감소
            return "A"
        elif total_score >= 0.60:  # 0.75에서 0.60으로 감소
            return "B"
        elif total_score >= 0.50:  # 0.60에서 0.50으로 감소
            return "C"
        elif total_score >= 0.40:  # 0.45에서 0.40으로 감소
            return "D"
        else:
            return "F"
...
def analyze_squat_video(
    ...
    while True:
        ...
        # 결과 표시
        # ─────────────────────────────────────────────────────────────────────────
        if display_countdown > 0 and squat_data["overall_grade"] is not None:
            # 스켈레톤 그리기
            if results.pose_landmarks:
            ...
            overlay_text = [
                ...
                f"Biomech: {squat_data.get('biomechanical_scores', {}).get('overall', 0):.2f}",
            ...
            # 등급에 따른 색상 결정
            grade_color = (0, 255, 0) if squat_data['overall_grade'] in ['A', 'B'] else (0, 0, 255)
            ...
            for i, text in enumerate(overlay_text):
                ...
                if i == 0:  # Phase 라인
                    cv2.putText(
                        frame, text, (20, y_pos),
                        font_face, font_scale + 0.2, phase_col, thickness + 1, lineType=cv2.LINE_AA
                    )
                else:
                    cv2.putText(
                        frame, text, (20, y_pos),
                        font_face, font_scale, grade_color, thickness, lineType=cv2.LINE_AA
                    )
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(script_dir, "squat2.mp4")
    output_path = os.path.join(script_dir, "squat_output2.mp4")
