    ...
    while True:
        ...
        if display_countdown > 0:
            ...
            overlay_text = [
                ...
                f"Current: {current_grade} ({current_score:.2f})  Best: {display_grade_str} ({display_score_str})",
                f"Cycle {current_cycle_num}: {current_cycle_best_grade} ({current_cycle_best_score:.2f})",
            ...
            # 현재 등급과 표시 등급의 색상
            current_grade_color = get_grade_color(current_grade)
            display_grade_color = get_grade_color(display_grade) if display_grade else (255, 255, 255)
            ...
            for i, text in enumerate(overlay_text):
                ...
                elif i == 4:  # Current/Best 등급 라인
                    # 현재 등급과 표시 등급을 다른 색상으로 표시
                    cv2.putText(
                        frame, f"Current: {current_grade} ({current_score:.2f})", (20, y_pos),
                        font_face, font_scale, current_grade_color, thickness, lineType=cv2.LINE_AA
                    )
                    cv2.putText(
                        frame, f"  Best: {display_grade_str} ({display_score_str})", (20 + 300, y_pos),
                        font_face, font_scale, display_grade_color, thickness, lineType=cv2.LINE_AA
                    )
                    continue
                elif i == 5:  # Cycle 등급 라인
                    cycle_grade_color = get_grade_color(current_cycle_best_grade)
                    text_color = cycle_grade_color
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(script_dir, "squat3.mp4")
    output_path = os.path.join(script_dir, "squat_output3.mp4")
