...
def main():
    ...
    pipeline = gstreamer_pipeline(capture_width=1280, capture_height=720)
    cap = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
    
    if not cap.isOpened():
        print("GStreamer 파이프라인을 통해 카메라를 열 수 없습니다.")
        return
    
    # 영상 저장을 위한 설정 (해상도는 파이프라인과 일치시킴)
    frame_width = 1280
    frame_height = 720
    fps = 30.0
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if 'knee' in angles:
                ...
                if stage == "down" or (stage == "up" and current_phase == "ASCEND"):
        ...
        # ------------------ 화면 표시 정보 수정 ------------------
        # 상단 정보 박스
        cv2.rectangle(image, (0,0), (frame_width, 80), (245,117,16), -1)
        
        # 타이머 표시
        cv2.putText(image, f'TIME: {remaining_time:.1f}s', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2, cv2.LINE_AA)
        
        # REPS
        cv2.putText(image, 'REPS', (int(frame_width * 0.3), 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
        cv2.putText(image, str(counter), (int(frame_width * 0.3), 65), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,255,255), 3, cv2.LINE_AA)
        
        # PHASE
        cv2.putText(image, 'PHASE', (int(frame_width * 0.55), 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
        cv2.putText(image, current_phase, (int(frame_width * 0.55), 65), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255,255,255), 3, cv2.LINE_AA)

        # LAST REP GRADE
        cv2.putText(image, 'GRADE', (int(frame_width * 0.8), 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
        cv2.putText(image, last_rep_grade, (int(frame_width * 0.8), 65), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,255,255), 3, cv2.LINE_AA)
        
        # 하단 안내 메시지
        cv2.rectangle(image, (0, frame_height-40), (frame_width, frame_height), (0,0,0), -1)
        cv2.putText(image, 'Press Q to quit early', (10, frame_height-15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1, cv2.LINE_AA)
        # ----------------------------------------------------
    ...
    if all_rep_results:
        ...
        print("분석된 스쿼트 동작이 없어 리포트를 저장하지 않았습니다.")
