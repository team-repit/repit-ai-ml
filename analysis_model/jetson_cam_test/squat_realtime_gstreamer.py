...
def save_report(report_path: str, total_reps: int, results: List[Dict]):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("GStreamer 실시간 스쿼트 자세 분석 리포트\n")
        f.write("="*30 + "\n")
...
def create_gstreamer_pipeline(camera_index=0, width=640, height=480, fps=30):
    """젯슨용 GStreamer 파이프라인 생성"""
    
    # 젯슨 Nano/Xavier용 파이프라인
    pipeline = (
    ...
    print(f"GStreamer 파이프라인: {pipeline}")
    return pipeline

def initialize_gstreamer_camera(camera_index=0):
    """GStreamer를 사용하여 카메라 초기화"""
    print("GStreamer 카메라 초기화 중...")
    ...
    for i in range(5):  # 0-4까지 테스트
        ...
        if os.path.exists(device_path):
            try:
                # 간단한 테스트로 카메라 사용 가능 여부 확인
                test_cap = cv2.VideoCapture(f"v4l2src device={device_path} ! videoconvert ! appsink", cv2.CAP_GSTREAMER)
                if test_cap.isOpened():
                    ret, frame = test_cap.read()
                    if ret:
                        available_cameras.append(i)
                        print(f"카메라 {i} (/dev/video{i}) 사용 가능")
                    test_cap.release()
            except Exception as e:
                print(f"카메라 {i} 테스트 실패: {e}")
    ...
    # GStreamer 파이프라인 생성
    pipeline = create_gstreamer_pipeline(camera_index)
    
    # OpenCV에서 GStreamer 파이프라인 사용
    cap = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
    
    if not cap.isOpened():
        print("GStreamer 파이프라인을 열 수 없습니다.")
        print("대안: 일반 V4L2 사용 시도...")
        # 대안으로 일반 V4L2 사용
...
def main():
    """GStreamer를 사용한 실시간 스쿼트 분석 메인 함수"""
    
    print("=== GStreamer 실시간 스쿼트 분석 (젯슨 최적화) ===")
    print("카메라 초기화 중...")
    
    # GStreamer 카메라 초기화
    cap = initialize_gstreamer_camera()
    
    if cap is None:
        print("카메라를 초기화할 수 없습니다.")
        print("\n문제 해결 방법:")
        print("1. 카메라가 연결되어 있는지 확인")
        print("2. GStreamer 설치 확인: sudo apt-get install libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev")
        print("3. 카메라 권한 확인: sudo usermod -a -G video $USER")
        print("4. 시스템 재부팅 후 다시 시도")
        print("5. 사용 가능한 장치 확인: v4l2-ctl --list-devices")
        return
    ...
    output_video_path = f"squat_gstreamer_analysis_{timestamp}.mp4"
    output_report_path = f"squat_gstreamer_report_{timestamp}.txt"
    ...
    while cap.isOpened():
        ...
        out.write(image)
        cv2.imshow('GStreamer Real-time Squat Analysis', image)
